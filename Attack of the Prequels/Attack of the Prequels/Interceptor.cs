...
{
    class Interceptor : EnemyShip
    {
        Player p;
        private int count = 1;
        ContentManager Content;
        int fireRate = Game1.rand.Next(63, 73);
        int count2;
        int count3 = 500;

        public Interceptor(ContentManager Content, Player p, Random rand)
            : base(Content, p)
        {

            this.p = p;

            velocity = p.initialVelocity + p.initialVelocity/2 ;
            this.Content = Content;
            health = 1;
            angleVelocity = 1f;
            scale=1500;
            model = Content.Load<Model>("tie3");
            count2 = count3 + 1;
       ...
       override public void MoveForward(ref Vector3 position, Quaternion rotationQuat, float speed)
        {
